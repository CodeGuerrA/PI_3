...
public class Pagamento extends javax.swing.JFrame {
    ...
    public Pagamento(int idMesa, double subtotal) {
        ...
        // Initialize the tableController object
        tableController = new PagamentoController(idMesa);
    }
    ...
    private void initComponents() {
        ...
        pagarevoltarServiço.setText("Pagar");
   ...
   TESTETextField6.setText("Pix");
    ...
    double serviceCharge = subtotal * 0.10; // Add 10% service charge
    double total = subtotal + serviceCharge;
    PagoTextField2.setText(String.format("%.1f", total)); // Format the Pago value to show only one decimal place
    ...
    // Calculate the troco for Pix
    double valorPago = Double.parseDouble(PagoTextField2.getText());
    double troco = valorPago - total;
    TrocoTextField4.setText(String.format("%.1f", troco));
    ...
    double serviceCharge = subtotal * 0.10; // Add 10% service charge
    double total = subtotal + serviceCharge;
    PagoTextField2.setText(String.valueOf(total));
    PagoTextField2.setEditable(true);
    PagoTextField2.setFocusable(true);
    // Desabilita a edição do campo de serviço
    ServiçoTextField5.setEditable(false);
    ...
    // Calculate the troco for Crédito
    double valorPago = Double.parseDouble(PagoTextField2.getText());
    double troco = valorPago - total;
    TrocoTextField4.setText(String.format("%.1f", troco));
    ...
    // Calculate the troco for Débito
    double valorPago = Double.parseDouble(PagoTextField2.getText());
    double troco = valorPago - total;
    TrocoTextField4.setText(String.format("%.1f", troco));
    ...
    private void PagoTextField2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_PagoTextField2ActionPerformed
      try {
        ...
        // Se a forma de pagamento selecionada for "Dinheiro", calcular o troco
        if (formaPagamentoSelecionada.equals("Dinheiro")) {
            double serviceCharge = subtotal * 0.10; // Add 10% service charge
            double total = subtotal + serviceCharge;
            double troco = valorPago - total;
            TrocoTextField4.setText(String.format("%.1f", troco));
        } else if (formaPagamentoSelecionada.equals("Crédito") || formaPagamentoSelecionada.equals("Débito")) {
            double serviceCharge = subtotal * 0.10; // Add 10% service charge
            double total = subtotal + serviceCharge;
            double troco = valorPago - total;
            TrocoTextField4.setText(String.format("%.1f", troco));
